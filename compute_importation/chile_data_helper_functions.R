get_adjusted_case_data_national <- function()
{
  ...
  under_reporting_path <- here("your_path")
...
chile_global_prevalence_estimates_function <- function(ecdc_data_arg, under_reporting_data_arg, date_arg)
{
    #Esta funcion se aplica fila a fila
    # Datearg es la fecha individual del recorrido
    #Date arg desde el 15 hasta el 29 de Junio
    date_arg <- as.Date(date_arg)
    ...
    new_case_estimates_recent <- ecdc_data_arg %>%
        ...
        #dplyr::filter(date_arg - 9 < date) %>%  #ANTES ESTABA SOLO ESTE FILTRO 
        #dplyr::mutate(country = stringr::str_replace_all(country, "_", " ")) %>%
        #dplyr::mutate(iso_code = countrycode::countrycode(country, "country.name", "iso3c", custom_match = c('Kosovo' = 'RKS'))) %>%
